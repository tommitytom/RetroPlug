...
([packages/native/](packages/native)) owns the emulator cores, realtime queues,
byte codecs, and OS paths/dialogs, while the application logic + React/LVGL UI
([packages/retroplug/](packages/retroplug)) is TypeScript over a single
synchronous `Backend` interface. It boots ROMs, plays multi-instance audio at
the host sample rate, drives the menu/tile UI, and persists projects. The shared
emulator/Project/codec core lives in [packages/native/src/](packages/native/src)
(compiled into `retroplug-core`); the host layer that composes it into the
plugin lives under [packages/native/src/host/](packages/native/src/host).
...
pnpm test:native # real host + emulator cores (the retroplug-host test binary)
...
```
packages/
  native/              the C++ host (one package). See spec/.
    src/               shared emulator/Project/codec core -> retroplug-core:
                       SameBoy + Mesen systems, Project, LSDj sav codec, transport
      host/            host layer -> retroplug-backend: engine, dsp runtime,
                       rpc backend (BackendFacade), system backends, input
    plugin/            the DPF plugin (clap / vst3 / vst2 / jack standalone)
    cli/  test/        standalone session runner + headless UI test harness
