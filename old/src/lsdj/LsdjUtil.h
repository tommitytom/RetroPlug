...
namespace rp::LsdjUtil {
	...
	static orb::PointT<uint32> pixelToTile(orb::PointT<uint32> pos) {
		return { pos.x / 8u, pos.y / 8u };
	}

	static bool tileToCursorPos(orb::PointT<uint32> tile, orb::PointT<uint8>& target) {
	...
	static bool pixelToCursorPos(orb::PointT<uint32> pos, orb::PointT<uint8>& target) {
