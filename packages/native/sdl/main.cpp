...
struct MidiClockSync {
    ...
    // a SYNC=MIDI cart used to start running at 120 BPM the moment it booted, with nothing plugged in and no
    // way to stop it. Non-sync use (mGB, an un-synced cart) never looked at the transport and is unaffected.
    bool   outPlaying() const { return active ? !stopped : manualPlaying; }
};
...
struct AppState {
    ...
    TjsHostRuntime host;
    // Three facets only, same as the plugin's channel — no debug, no audio-driver. See
    // host/HostServices.hpp for why those two are not members rather than merely unmounted.
    HostServices   svc;
    ...
    // The local transport (Instance menu > Transport), across the UI/audio thread boundary. clockSync itself
    // is audio-thread-only, so the menu can't poke it: the UI writes the REQUEST, the callback folds it into
...
bool readReady(TjsHostRuntime& host) { return jsReadReady(host.context()); }

// Call globalThis[name](arg?) on the control-plane context (main thread). Returns the string result, or
// "" for non-string / missing / exception. Shared with PluginDSP::callGlobal.
//
// `ready` is true unconditionally here, and that is not laziness: bootControlPlane returns false if the
// host fails to initialise OR the bundle never signals ready, and main exits 1 on either. An unready SDL
// host is a dead process, so by the time anything calls this there is nothing to guard against. The
// plugin cannot take that route - a DAW scan must degrade rather than exit - which is why the shared
// body carries the guard and this caller opts out of it explicitly.
std::string callGlobal(TjsHostRuntime& host, const char* name, const char* arg) {
    JsGlobals g{ host.context(), /*ready=*/true,
                 [](const char* n, const char* detail) {
                     std::fprintf(stderr, "[retroplug-sdl] %s threw: %s\n", n, detail);
                 } };
    return jsCallGlobal(g, name, arg);
}

bool hasGlobalFunction(JSContext* ctx, const char* name) { return jsHasGlobalFunction(ctx, name); }
...
JSValue jsOpenPath(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    if (argc >= 1) {
        ...
        if (p && *p) openPathInFileManager(p);
...
// __rp_setTransport(playing): start/stop the local transport. Takes effect on the next audio block; with an
// external clock master present it is recorded but overridden (see jsGetTransport).
...
void renderAudioBlock(AppState& a, float* out, int frames) {
    ...
    a.svc.invoker.drainInto(a.svc.engine);
    ...
    for (auto& m : a.midiScratch) {
        ...
        a.svc.engine.stageMidi(0, std::move(m.bytes));
    ...
    for (auto& m : a.launchpadScratch)
        if (!m.empty()) a.svc.engine.stageControllerMidi(std::move(m));
    a.svc.engine.setControllerConnected(a.launchpadHost.link().isConnected());
    ...
    a.transportExternal.store(a.clockSync.active, std::memory_order_relaxed);
    a.transportBpm.store(static_cast<float>(bpm), std::memory_order_relaxed);
    a.svc.engine.setBpm(bpm);            // 120 free-run until an external clock master appears
    a.svc.engine.setTransport(playing);  // then locks to its tempo + start/stop
    ...
    a.svc.engine.processBlock(static_cast<std::uint32_t>(frames), a.audioPtrs.data(), static_cast<std::size_t>(N));
    ...
    for (const auto& mo : a.svc.engine.midiOut())
        if (!mo.data.empty()) a.midi.send(mo.data.data(), mo.data.size());
    a.svc.engine.clearMidiOut();
    ...
    for (const auto& co : a.svc.engine.controllerOut())
        if (!co.empty()) a.launchpadHost.link().pushOutput(co.data(), co.size());
    a.svc.engine.clearControllerOut();
...
std::string audioCfgPath(AppState& a) { return a.svc.host.configDir() + "/audio.json"; }
...
std::string midiCfgPath(AppState& a) { return a.svc.host.configDir() + "/midi.json"; }
...
void reconfigureAudio(AppState& a, int sampleRate, int blockSize, int channels, int hostApi, const std::string& device) {
    ...
    a.svc.invoker.setAudioThreadOwns(false);
    a.svc.invoker.drainInto(a.svc.engine);
    ...
    a.svc.engine.setSampleRate(sampleRate); // re-rate live cores (safe — audio stopped)
    openAudio(a);                       // opens a real stream, or leaves it muted (numOutputs still sized)
    a.svc.invoker.setAudioThreadOwns(true);
...
bool bootControlPlane(AppState& a) {
    ...
    registerCoreBackends(a.svc.factory);
    ...
    registerHostRpc(*a.server, a.svc.host);
    registerEmulatorRpc(*a.server, a.svc.engineSvc);
    registerDspKernelRpc(*a.server, a.svc.engineSvc);
...
int main(int argc, char** argv) {
    ...
    app.svc.engine.setCoreByteSink([a = &app](std::uint32_t frame, const std::uint8_t* data,
    ...
    app.svc.invoker.setAudioThreadOwns(true);
#if defined(__linux__)
    ...
    app.svc.invoker.setAudioThreadOwns(false);
    app.svc.invoker.drainInto(app.svc.engine);
    app.svc.invoker.reclaimReleased();
