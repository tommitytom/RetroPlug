...
// The OS-native file picker (Load/Save/Export/Output-Dir). SDL2 has no file-dialog API, so we back the
// __rp_openFileBrowser seam (the plugin gets it free from DPF) with this MIT header-only lib: it fork/execs
// zenity/kdialog on Linux, IFileDialog on Windows, Cocoa/AppleScript on macOS. Header-only — the impl is
// inline, pulled in by this single TU. pfd::settings::available() gates the bind so a helper-less host
// (the muOS handheld) transparently keeps the in-app browser.
...
struct AppState {
    ...
    // An in-flight OS-native file dialog (opened by __rp_openFileBrowser). Async: opened here, polled each
    // frame in the main loop (pollFileDialog), the pick delivered to JS on ready. One at a time — the TS
    // FileSelection flow awaits sequentially (mirrors PluginUI's "one dialog is ever in flight"). unique_ptr
    // sidesteps any move/copy constraints on the pfd types; whichever is set is the active dialog.
    std::unique_ptr<pfd::open_file>     dlgOpen;
    std::unique_ptr<pfd::save_file>     dlgSave;
    std::unique_ptr<pfd::select_folder> dlgFolder;
    bool dialogActive() const { return dlgOpen || dlgSave || dlgFolder; }
...
// dialog (bound only where pfd::settings::available()). Async — stashes the dialog in AppState; pollFileDialog
// delivers the pick. Mirrors PluginUI::requestFileBrowser's arg contract: `patterns` is a whitespace-separated
// glob list; `directory` picks a FOLDER (the render "Output Dir"); `saving` seeds the save name with defaultName.
JSValue jsOpenFileBrowser(JSContext* ctx, JSValueConst, int argc, JSValueConst* argv) {
    if (!g_app || argc < 6) return JS_UNDEFINED;
    if (g_app->dialogActive()) return JS_UNDEFINED; // one dialog at a time (the TS flow awaits sequentially)
    ...
    const std::string title    = arg(0);
    const std::string patterns = arg(1);
    const bool        saving   = JS_ToBool(ctx, argv[2]) == 1;
    const std::string defName  = arg(3);
    const std::string startDir = arg(4);
    const bool        directory = JS_ToBool(ctx, argv[5]) == 1;

    const std::string caption = title.empty() ? "Open" : title;
    // pfd filters are {label, patterns} pairs. Offer the caller's globs first, then an all-files escape hatch.
    std::vector<std::string> filters;
    if (!patterns.empty()) { filters.push_back("Files"); filters.push_back(patterns); }
    filters.push_back("All Files"); filters.push_back("*");

    if (directory) {
        g_app->dlgFolder = std::make_unique<pfd::select_folder>(caption, startDir);
    } else if (saving) {
        // Seed the save box with the suggested filename under startDir (pfd takes a full default_path).
        std::string defPath = defName;
        if (!startDir.empty() && !defName.empty()) {
            const char sep = startDir.back() == '/' ? '\0' : '/';
            defPath = sep ? startDir + sep + defName : startDir + defName;
        } else if (!startDir.empty()) {
            defPath = startDir;
        }
        g_app->dlgSave = std::make_unique<pfd::save_file>(caption, defPath, filters);
    } else {
        g_app->dlgOpen = std::make_unique<pfd::open_file>(caption, startDir, filters);
    }
    return JS_UNDEFINED;
}

// Poll the in-flight OS file dialog once per frame; when the helper process finishes, deliver the pick (or a
// cancel = empty) into JS and clear the slot. Non-blocking (ready(0)); no dialog → cheap no-op.
void pollFileDialog(AppState& a) {
    JSContext* ctx = a.ui.getContext();
    if (!ctx) return;
    if (a.dlgOpen && a.dlgOpen->ready(0)) {
        auto sel = a.dlgOpen->result();
        a.dlgOpen.reset();
        deliverFileBrowserResult(ctx, sel.empty() ? std::string() : sel.front());
    } else if (a.dlgSave && a.dlgSave->ready(0)) {
        std::string sel = a.dlgSave->result();
        a.dlgSave.reset();
        deliverFileBrowserResult(ctx, sel);
    } else if (a.dlgFolder && a.dlgFolder->ready(0)) {
        std::string sel = a.dlgFolder->result();
        a.dlgFolder.reset();
        deliverFileBrowserResult(ctx, sel);
    }
}
...
void installWindowHooks(JSContext* ctx) {
    ...
    if (pfd::settings::available() || std::getenv("RETROPLUG_SDL_DIALOG_SELFTEST"))
...
bool setupUi(AppState& a) {
    ...
    // Headless proof that the OS-dialog seam is wired (the bug was the SDL host never binding it). Asserts
    // __rp_openFileBrowser is bound, then drives one deliver through the __rp_onFileBrowserResult resolver to
    // prove the C++→JS callback path — under the dummy/offscreen driver, which can't open a real dialog.
    if (const char* selftest = std::getenv("RETROPLUG_SDL_DIALOG_SELFTEST")) {
        const bool bound = ctx && hasGlobalFunction(ctx, "__rp_openFileBrowser");
        std::fprintf(stderr, "[retroplug-sdl] dialog self-test: __rp_openFileBrowser %s; delivering %s\n",
                     bound ? "BOUND" : "MISSING", selftest);
        if (ctx) deliverFileBrowserResult(ctx, selftest);
    }
    return true;
}
