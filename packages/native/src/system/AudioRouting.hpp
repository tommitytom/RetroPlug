...
//
// Both split modes are single-system only; the Engine gates them
// (systemCount()==1) and falls back to Stereo for any other project.
//
// Modes 0..2 fan MANY systems across the fixed pairs (MultiOutRouter); modes 3/4
// split ONE system's channels (ChannelSplitRouter, whose laneStride is 2 for
// stereo streams and 1 for mono ones). The routing value is owned by TS and
// pushed into Engine::audioRouting_ via the SetAudioRouting RPC; the Engine
