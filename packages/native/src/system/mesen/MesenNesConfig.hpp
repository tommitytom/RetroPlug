...
struct MesenNesConfig {
    ...
    // Cartridge-accuracy switches (TS enums, 0 = "chip", 1 = "n8"). Default to the documented chip
    // behaviour; select "n8" to match an Everdrive N8 Pro's FPGA cores, which measurably differ - so
    // software developed against that cartridge (EverMIDI) sounds the same in the emulator. Live knobs.
