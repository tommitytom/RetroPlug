...
// UI thread → DSP thread. A 1024-entry SpscRing of POD Command records — handles
// chord-style key spam comfortably without ever touching the heap. An empty
// derived class (not a `using` alias) so `class CommandQueue;` forward
// declarations elsewhere stay valid.
class CommandQueue : public SpscRing<Command, 1024> {};
