...
UiHarness::UiHarness(std::uint32_t width, std::uint32_t height)
    : core_(width, height) { registerCoreBackends(factory_); }
...
bool UiHarness::boot() {
    ...
    server_    = std::make_unique<BackendRpcServer>(*transport_, rpcpp::QuickJSCodec{ctx});
