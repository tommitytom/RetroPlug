...
// packages/retroplug-greenfield/test-native/_diag-aboy2.test.ts
var ABOY = "/workspaces/resources/roms/lsdj/lsdj9_3_3-arduinoboy.gb";
var FULL = `var was=false; function onBlock(input){ if(input.transportPlaying&&!was) pushSerialIn(0,0xFA); was=input.transportPlaying; eachTick(24,function(t,o){ pushSerialIn(o,0xF8); }); }`;
var CLOCK_ONLY = `function onBlock(input){ eachTick(24,function(t,o){ pushSerialIn(o,0xF8); }); }`;
var sav = () => savFromJson(JSON.stringify({
  workingSong: {
    formatVersion: 22,
...
var build = () => be.constructSystem({ romPath: ABOY, embeddedRom: "", savPath: null, statePath: null, sramBytes: sav().slice().buffer, lsdjSyncMode: "Off" });
test("aboy min: negative (no DSP) then 0xF8-only", () => {
  ...
  expect(dsp.loadScript(dsp.compileScript(CLOCK_ONLY))).toBeTruthy();
  audio.dspAttach(id);
  const clockOnly = rms(audio.renderAudio(3e3));
  console.log(`[aboy min] neg(noDSP)=${neg.toFixed(5)} clockOnly(0xF8)=${clockOnly.toFixed(5)}`);
  audio.dspAttach(0);
  audio.setTransport(false);
  be.removeSystem(id);
});
test("aboy min: FULL (0xFA+0xF8), DSP attached before transport", () => {
  if (!be.fileExists(ABOY)) {
    console.log("# SKIP");
    return;
  }
  const id = build();
  audio.renderAudio(6e3);
  expect(dsp.loadScript(dsp.compileScript(FULL))).toBeTruthy();
  audio.dspAttach(id);
  audio.setBpm(120);
  audio.setTransport(true);
  const full = rms(audio.renderAudio(3e3));
  console.log(`[aboy min] FULL(0xFA+0xF8)=${full.toFixed(5)}`);
  audio.dspAttach(0);
  audio.setTransport(false);
  be.removeSystem(id);
});
