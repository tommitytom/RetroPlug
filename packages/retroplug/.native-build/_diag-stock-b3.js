...
// packages/retroplug-greenfield/test-native/_diag-stock-b3.test.ts
var STOCK = "/workspaces/resources/roms/lsdj/lsdj9_4_2.gb";
var START = 7;
var CLOCK = `function onBlock(input){ eachTick(24, function(t,o){ pushSerialIn(o,0xF8); }); }`;
function sav() {
  const u = new Uint8Array(savFromJson(JSON.stringify({
    workingSong: {
      formatVersion: 22,
...
test("stock byte-3 MIDI, role=Off, arm, DSP eachTick->0xF8 (negative vs positive)", () => {
  if (!be.fileExists(STOCK)) {
    console.log("# SKIP");
    return;
  }
  const id = be.constructSystem({ romPath: STOCK, embeddedRom: "", savPath: null, statePath: null, sramBytes: new Uint8Array(sav()).buffer, lsdjSyncMode: "Off" });
  ...
  audio.dspAttach(id);
  const pos = rms(audio.renderAudio(3e3));
  audio.screenshot(id, "/tmp/stock_b3_playing.png");
  console.log(`[stock-b3] neg(noDSP)=${neg.toFixed(5)} pos(DSP)=${pos.toFixed(5)}`);
});
