...
// packages/retroplug/test-native/lsdj-sync-pattern.test.ts
...
test("LSDj link-cable sync: the follower plays in lockstep with the leader (per-system audio)", () => {
  ...
  expect(construct(be, follower, "Lsdj")).toBeTruthy();
  ...
  expect(r1 > 1e-3).toBeTruthy();
  expect(r1 > r0 * 0.5).toBeTruthy();
  expect(r1 < r0 * 2).toBeTruthy();
});
