...
// packages/retroplug/test-ui/resize.test.ts
test("adding a second instance requests a window resized to fit the grid", () => {
  const calls = [];
  globalThis.__rp_setWindowSize = (w, h) => calls.push([w, h]);
  ...
  expect(dual[0]).toBe(single[0] * 2);
  expect(dual[1]).toBe(single[1]);
});
