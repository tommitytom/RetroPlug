...
const N8_LOAD_HELP = [
  ...
  "",
  "  HAZARD - reading CART memory takes the bus off the console. Every option that touches PRG / CHR /",
  "  SRM (--dump-chr, --patch-chr, --patch-prg, --dump-sram, --show-song) is a USB DMA, and the FPGA",
  "  hands that memory controller to the DMA for the WHOLE command (edn8-pro-pub everdrive.sv:",
  "  `chr = dma.req_chr ? dma.mem : mao.chr`, and the PPU reads 0x00 meanwhile). A timing-sensitive",
  "  game can fail to survive that - measured on a mapper-30 CHR-RAM ROM, where a single 16-byte read",
  "  is enough to wedge it until a power-cycle, while an NROM CHR-ROM game is unaffected. Reads of the",
  "  SYSTEM region are NOT a DMA and never touch the cart bus, so --sniff / --info are always safe;",
  "  prefer them for polling a running game.",
  "",
