// `retroplug-cli risa-rom` — inspect, extract and edit the static assets (DPCM sample kits, themes, fonts)
// inside a risa (NES/MMC5) `.nes` ROM, headlessly. The risa twin of ./lsdj-rom.ts, backed by the pure-TS
// src/risa/rom module; this is just the CLI surface (arg parsing + file I/O + WAV/JSON output). Kit compile
// is native (s.audio.compileDmc — the risa analog of compileKit); sample splicing re-packs existing DPCM +
// a freshly-compiled slot via assembleKitBank (byte-identical to a whole-kit recompile).
//
// ONE JSON schema drives the editing verbs (see RISA_ROM_HELP): a manifest of kit/theme/font entries, each
// of which BUILDS/IMPORTS from a file or TWEAKS metadata; `build-kit` reads a single, slotless kit entry.
//
//   retroplug-cli risa-rom info          <rom> [--json]
//   retroplug-cli risa-rom extract       <rom> <outDir> [--rate N]
//   retroplug-cli risa-rom patch         <rom> <manifest.json> <out>       (the whole-ROM manifest)
//   retroplug-cli risa-rom build-kit     <kit.json> <out.rkit> [flags]     (native compile → .rkit file)
//   retroplug-cli risa-rom import-sample <rom> <kit> <audio> [flags]       (compile one + splice)
//   retroplug-cli risa-rom remove-sample <rom> <kit> <slot> [--out rom]
//   retroplug-cli risa-rom export-theme  / import-theme   (.rit palette-role JSON)
//   retroplug-cli risa-rom export-font   / import-font    (.chr raw 8 KB CHR banks)
//   retroplug-cli risa-rom export-kit    / import-kit     (.rkit raw 8 KB DPCM banks)
...
function openRom(s: Session, path: string): RisaRom {
  const bytes = s.backend.readFile(path);
  if (!bytes) throw new Error(`cannot read ROM: ${path}`);
  const rom = RisaRom.fromBytes(bytes);
  if (!rom.isRisa) throw new Error(`not a recognised risa ROM (size=${bytes.length})`);
  return rom;
}
...
// Per-sample shaping. risa's DMC codec skips dither (4-bit only) — so gain/filter only. --no-normalize maps
...
// The kit's 16 slots as re-packable AssembleSlots (null where empty), for splice/rename.
function kitSlots(rom: RisaRom, kitIndex: number): { name: string; slots: (AssembleSlot | null)[] } {
...
function applyKitRenames(rom: RisaRom, slot: number, newName: string | undefined, renames: { index: number; name: string }[] | undefined): void {
...
function romToJson(rom: RisaRom): unknown {
...
function info(s: Session, args: string[]): void {
  ...
  if (!romPath) throw new Error("usage: risa-rom info <rom> [--json]");
...
function extract(s: Session, args: string[]): void {
  ...
  if (!romPath || !outDir) throw new Error("usage: risa-rom extract <rom> <outDir> [--rate N]");
...
function buildKit(s: Session, args: string[]): void {
  ...
  if (!specPath || !outKit) throw new Error("usage: risa-rom build-kit <kit.json> <out.rkit> [effect flags]");
...
function importSample(s: Session, args: string[]): void {
  ...
  if (!romPath || kitStr == null || !audio) throw new Error("usage: risa-rom import-sample <rom> <kit> <audio> [--slot N] [--name X] [--out rom] [flags]");
...
function removeSample(s: Session, args: string[]): void {
  ...
  if (!romPath || kitStr == null || slotStr == null) throw new Error("usage: risa-rom remove-sample <rom> <kit> <slot> [--out rom]");
  const kitIndex = parseInt(kitStr, 10);
  ...
  slots[slot] = null; // empty the slot — its index is preserved (risa slots are index-addressed)
...
function exportTheme(s: Session, args: string[]): void {
  ...
  if (!romPath || idxStr == null || !out) throw new Error("usage: risa-rom export-theme <rom> <index> <out.rit>");
  const rom = openRom(s, romPath);
...
function importTheme(s: Session, args: string[]): void {
  ...
  if (!romPath || !file || idxStr == null) throw new Error("usage: risa-rom import-theme <rom> <in.rit> <index> [--out rom]");
  const rom = openRom(s, romPath);
...
function exportFont(s: Session, args: string[]): void {
  const [romPath, idxStr, out] = positionals(args);
  if (!romPath || idxStr == null || !out) throw new Error("usage: risa-rom export-font <rom> <index> <out.chr>");
  const rom = openRom(s, romPath);
...
function importFont(s: Session, args: string[]): void {
  const [romPath, file, idxStr] = positionals(args);
  if (!romPath || !file || idxStr == null) throw new Error("usage: risa-rom import-font <rom> <in.chr> <index> [--out rom]");
  const rom = openRom(s, romPath);
...
function exportKit(s: Session, args: string[]): void {
  const [romPath, idxStr, out] = positionals(args);
  if (!romPath || idxStr == null || !out) throw new Error("usage: risa-rom export-kit <rom> <index> <out.rkit>");
  const rom = openRom(s, romPath);
...
function importKit(s: Session, args: string[]): void {
  const [romPath, file, idxStr] = positionals(args);
  if (!romPath || !file || idxStr == null) throw new Error("usage: risa-rom import-kit <rom> <in.rkit> <index> [--out rom]");
  const rom = openRom(s, romPath);
  ...
  rom.setKit(parseInt(idxStr, 10), data); // dual-writes the ROM's kit-metadata mirror
  const out = flag(args, "--out") ?? romPath;
  if (!s.backend.writeFileAtomic(out, rom.bytes())) throw new Error(`write failed: ${out}`);
...
function patchManifest(s: Session, args: string[]): void {
  ...
  if (!romPath || !manifestPath || !outRom) throw new Error("usage: risa-rom patch <rom> <manifest.json> <out>");
  const rom = openRom(s, romPath);
...
const RISA_ROM_HELP = [
  "usage: retroplug-cli risa-rom <subcommand> ...",
  "",
  "  info          <rom> [--json]                     theme / font / kit inventory",
  "  extract       <rom> <outDir> [--rate N]          dump each kit sample to a mono WAV + themes/fonts + rom.json",
...
export const risaRomTool: CliTool = {
  name: "risa-rom",
  summary: "inspect / extract / metadata-patch risa ROM kits, themes and fonts",
  help: RISA_ROM_HELP,
