...
// (a replaced DMC kit / CHR font) — the EverMIDI twin of ./risaAssetsRole.ts. It carries NO DSP behaviour:
// the base `.nes` on disk is never touched; the overrides are folded into the base ROM in memory at
// CONSTRUCT time (the onConstruct hook), so `effective ROM = base ROM + overrides`, rebuilt on every load.
// The override manifest is the persisted source of truth (it round-trips through the project's role config).
// KITS and FONTS are binary, so they LINK their bank file on disk by path — a pre-built 8 KB `.rkit` DMC
// bank / an 8 KB `.chr` CHR bank, read at construct (kit compilation is offline, like risa). A kit override
// with `erase: true` empties the slot instead. Applying reuses the pure-TS patcher (src/evermidi/rom).
// (No THEMES yet — EverMIDI has no ROM-side theme table; that needs a ROM retrofit.)
import type { RoleRegistry, ConstructCaps } from "./systemRoles";
import type { ConstructSpec } from "./backend";
import { z } from "./configSchema";
import { KIT_BANK_SIZE, CHR_BANK_SIZE, isBankPopulated } from "./risa/rom";
