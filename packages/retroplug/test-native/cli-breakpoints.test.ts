...
// expression gates the break. $8D98 is the LDA $40F1 inside bliptoaster's idle loop (LDA $40F1 / AND #$80 /
// BNE) - the MIDI FIFO poll it runs every pass, so an execute break there and a $40F1 read watchpoint fire
// on the SAME instruction; it's also the `midiIdleLoop` label in the committed resources/roms/bliptoaster.dbg.
// Y is 3 there (sampled stable across separate hits), so "Y == 3" matches and "Y == 0" cannot.
...
test("breakpoints fire on a real NES: execute PC, FIFO read watchpoint, cycle cap, conditions", () => {
  ...
  expect(s.backend.setBreakpoints(id, [{ type: "execute", start: 0x8d98 }])).toBeTruthy();
  const exec = s.backend.runUntilBreak(id, 5_000_000);
  expect(exec.broke).toBeTruthy();
  expect(exec.pc).toBe(0x8d98);
  ...
  s.backend.setBreakpoints(id, [{ type: "execute", start: 0x8d98, condition: "1 == 0" }]);
  expect(s.backend.runUntilBreak(id, 500_000).broke).toBeFalsy();

  // Y is 8 throughout the FIFO poll — this condition matches and fires at $A4B3.
  s.backend.setBreakpoints(id, [{ type: "execute", start: 0x8d98, condition: "Y == 3" }]);
  const cond = s.backend.runUntilBreak(id, 5_000_000);
  expect(cond.broke).toBeTruthy();
  expect(cond.pc).toBe(0x8d98);
});
