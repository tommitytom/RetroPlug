...
test("scan: a ROM is missing when its path is absent and no blob is embedded", () => {
  const c = cfg([{ platform: "gb", core: "sameboy", romPath: "/roms/a.gb" }]);
...
test("scan: present ROM / embedded ROM / bundled rom-blob are all OK", () => {
  const present = cfg([{ platform: "gb", core: "sameboy", romPath: "/roms/a.gb" }]);
  expect(scanMissingFiles(present, NO_BLOBS, (p) => p === "/roms/a.gb")).toEqual([]);

  const embedded = cfg([{ platform: "gb", core: "sameboy", embeddedRom: "mgb" }]);
  expect(scanMissingFiles(embedded, NO_BLOBS, () => false)).toEqual([]); // baked in

  const bundled = cfg([{ platform: "gb", core: "sameboy", romPath: "/roms/a.gb" }]);
...
test("scan: an explicit savPath with no file + no blob is missing; an empty savPath is allowed", () => {
  const withOverride = cfg([{ platform: "gb", core: "sameboy", romPath: "/roms/a.gb", savPath: "/saves/x.sav" }]);
  ...
  const noOverride = cfg([{ platform: "gb", core: "sameboy", romPath: "/roms/a.gb" }]);
  expect(scanMissingFiles(noOverride, NO_BLOBS, (p) => p === "/roms/a.gb")).toEqual([]);
});

test("scan: multiple systems report by config index", () => {
  const c = cfg([
    { platform: "gb", core: "sameboy", romPath: "/roms/a.gb" }, // present
    { platform: "gb", core: "sameboy", romPath: "/roms/b.gb" }, // missing
  ]);
...
test("relinkInConfig: repoints rom / sram in place; false for a bad index", () => {
  const c = cfg([{ platform: "gb", core: "sameboy", romPath: "/old/a.gb", savPath: "/old/a.sav" }]);
...
test("autoFindSiblings: one located folder fixes the rest by basename", () => {
  const c = cfg([
    { platform: "gb", core: "sameboy", romPath: "/old/a.gb" },
    { platform: "gb", core: "sameboy", romPath: "/old/b.gb" },
  ]);
