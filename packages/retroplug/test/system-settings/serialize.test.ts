...
test("customized settings + role round-trip through build → serialize → parse → adopt", () => {
  ...
  s1.setRoleConfig(id, "sameboy", { model: "sgb" });
  s1.setGain(id, -6);
  ...
  expect(v.roles[0].config.model).toBe("sgb"); // restored (stored role wins over defaults)
});
...
test("a customized NES (mesen) role round-trips through serialize → parse → adopt", () => {
  ...
  const cfg = buildConfig(DEFAULT_SETTINGS, s1.systems());
  expect(cfg.systems[0].roles).toEqual([{ kind: "mesen", config: { region: "pal", removeSpriteLimit: false, enableFm: true, apuLatencyMs: 1.4, s5bNoise: "n8", mmc5PhaseReset: "n8", expansionVolume: 100, channelExportMode: "mix", sdRoot: "", fifo: "instant", fifoDepth: 0, fifoBytesPerSecond: 0 } }]);
  ...
  expect(s2.view()[0].roles[0].config.region).toBe("pal"); // restored
});
