...
test("a customized NES (mesen) role round-trips through serialize → parse → adopt", () => {
  ...
  const cfg = buildConfig(DEFAULT_SETTINGS, s1.systems());
  expect(cfg.systems[0].roles).toEqual([{ kind: "mesen", config: { region: "pal", removeSpriteLimit: false, enableFm: true, apuLatencyMs: 1.4, s5bNoise: "chip", mmc5PhaseReset: "chip", channelExportMode: "mix" } }]);
