...
test("a NES system auto-gets the mesen system role; editing region crosses to the emulator", () => {
  ...
  expect(v.roles).toEqual([{ kind: "mesen", config: { region: "auto", removeSpriteLimit: false, enableFm: true, apuLatencyMs: 1.4, channelExportMode: "mix" } }]);
  ...
  expect(be.applyRoleCalls[be.applyRoleCalls.length - 1]).toEqual({
    id,
    kind: "mesen",
    config: { region: 2, removeSpriteLimit: false, enableFm: true, apuLatencyMs: 1.4, channelExportMode: 0 }, // whole role config crosses (system-category, native-encoded)
  });
});
