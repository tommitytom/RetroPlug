...
function PluginUI() {
    ...
    useKeyboard(useCallback((key: number, press: boolean) => {
        if (key === KEY_ESCAPE) {
            ...
            // Menu open: defer to Menu's own Esc handler — it owns
            // capture / prompt mode and decides whether Esc cancels
            // those or closes the menu. Calling onClose() from inside
            // Menu eventually flows through closeMenu() below.
            if (menuOpenRef.current) return;
            // Menu closed: open it, anchored to the focused tile.
            setMenuSystemId(focusedIdRef.current);
            setMenuOpen(true);
            return;
        }
    ...
    useGamepadButton(useCallback((pad: number, buttonName: string, press: boolean) => {
        // Menu open: bindings submenu's capture mode owns gamepad events;
        // even when not capturing, suppressing here avoids pressing the
        // emulated joypad while the user is navigating the menu.
        if (menuOpenRef.current) return;
    ...
    // rows + Save/Revert/Save-As/Rename/Duplicate/Delete actions.
    const keyboardEditor = useBindingsEditor(
        "keyboard", availableProfiles, activeKeyboardBindings);
    const gamepadEditor  = useBindingsEditor(
        "gamepad",  availableProfiles, activeGamepadBindings);
