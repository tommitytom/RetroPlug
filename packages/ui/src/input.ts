// RetroPlug input layer — TypeScript side. (Game-Boy-specific; lives in the
// product, not the dpf.js framework.)
//
// The single source of truth for turning host input into Game Boy button
// presses: the GameboyButton enum (mirrors src/system/InputTypes.hpp /
// SameBoy's GB_key_t — values must stay in sync), the runtime-configurable
// keyboard + gamepad bindings, and the React hooks that subscribe to the input
// event channels the plugin's C++ emits.
//
// src/PluginUI.cpp emits those channels through the generic dpf.js event bus
// (lvgljs.on/off): "key" (DPF key code + press), "mouse", and the "gamepad-*"
// channels (see src/GamepadManager.cpp). DPF key codes match dpf.js's
// deps/dpf/dgl/Base.hpp; ASCII keys arrive as their unicode codepoints.
//
// Bindings are runtime-configurable: PluginUI.tsx calls plugin.getUserConfig()
// on mount and pipes the result through installBindings(); a subsequent
// "user-config-changed" event re-fetches and re-installs. The defaults below
// keep the bundle working before installBindings() runs.
