...
def  main():
  ...
  installer = "\installer\RetroPlug Installer.exe"
   
  if demo:
    installer = "\installer\RetroPlug Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RetroPlug manual.pdf" 
  ]
  ...
  ZIPNAME = "RetroPlug-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RetroPlug-v" + FULLVERSIONSTR + "-win-demo.zip"
