...
> **Legacy is being removed.** An older build still ships alongside greenfield
...
> only because greenfield has not yet reached full parity. It is on its way out —
> greenfield will become the sole build and drop the `-greenfield` suffix. This
> `spec/` never describes legacy internals as current; the switchover and the
...
surface greenfield does not use — a naming wart flagged for re-homing in
...
| [07-migration.md](07-migration.md) | The switchover to greenfield-as-sole-build: the feature gap, the shared-vs-legacy C++ map, the rename/delete checklist, and deletion risks. |
