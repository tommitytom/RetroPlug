...
It compiles into one static library, [`retroplug-backend`](../packages/native/CMakeLists.txt#L13),
...
[`BackendFacade`](../packages/native/src/BackendFacade.hpp#L24) is **one object per
RPC server**. It owns the shared [`Engine`](../packages/native/src/Engine.hpp#L35),
the [`SystemFactory`](../packages/native/src/SystemFactory.hpp#L38), and the
[`QueuedInvoker`](../packages/native/src/EngineInvoker.hpp#L28), and has-a three
concern-separated services that hold references to them:

| Sub-object | Concern | Header |
|---|---|---|
| `host_` | filesystem / config / zip (pure, stateless) | [`HostRpcService.hpp`](../packages/native/src/HostRpcService.hpp) |
| `engine_svc_` | emulator lifecycle / reads / kernel / MIDI / transport | [`EngineRpcService.hpp`](../packages/native/src/EngineRpcService.hpp) |
| `driver_` | background audio thread + observation atomics (test host only) | [`AudioDriverRpcService.hpp`](../packages/native/src/AudioDriverRpcService.hpp#L17) |

Every wire method on the facade forwards a single line to its sub-object (see
[`BackendFacade.hpp:31-71`](../packages/native/src/BackendFacade.hpp#L31)). The
**pointer-to-member name IS the wire name** — rpcpp derives each method identifier from the
member pointer registered in
[`registerBackendRpcMethods`](../packages/native/src/BackendRpcRegistration.hpp#L10) —
...
([`BackendFacade.hpp:96-102`](../packages/native/src/BackendFacade.hpp#L96)).
...
[`BackendFacade.hpp:73-91`](../packages/native/src/BackendFacade.hpp#L73)). These
...
[`BackendTypes.hpp`](../packages/native/src/BackendTypes.hpp).
...
| `writeFile` | `(string, Bytestring) → bool` | creates parent dirs on demand ([`:81`](../packages/native/src/HostRpcService.cpp#L81)) |
| `writeFileAtomic` | `(string, Bytestring) → bool` | tmp + rename ([`:94`](../packages/native/src/HostRpcService.cpp#L94)) |
| `fileExists` / `rename` / `deleteFile` | `→ bool` | |
| `listDir` | `(string dir) → vector<string>` | filenames only |
| `drainChangedPaths` | `→ vector<string>` | **always empty** — no file watcher in this host ([`:133`](../packages/native/src/HostRpcService.cpp#L133)) |
| `canonicalize` | `(string) → string` | `weakly_canonical`, falls back to input |
| `configDir` | `→ string` | per-OS; **reimplemented** to avoid linking `UserConfig`/efsw ([`:23`](../packages/native/src/HostRpcService.cpp#L23)) |
| `zip` | `(vector<BackendZipInput>) → Bytestring` | miniz, no-copy add ([`:147`](../packages/native/src/HostRpcService.cpp#L147)) |
| `unzip` | `(Bytestring) → vector<BackendZipEntry>` | |
| `savFromJson` | `(string json) → Bytestring` | lenient LSDj sav authoring, then `encodeSav` ([`:163`](../packages/native/src/HostRpcService.cpp#L163)) |
...
|---|---|---|
| `constructSystem` | `(BackendConstructSpec) → bool` | TS-owned `spec.id`; `factory_.build` → `registry().claim` → `invoker_.replaceSystem`/`adoptSystem`. Returns "did it build" ([`:71`](../packages/native/src/EngineRpcService.cpp#L71)) |
| `removeSystem` | `(uint32 id) → bool` | `invoker_.removeSystem`; no threading branch ([`:92`](../packages/native/src/EngineRpcService.cpp#L92)) |
| `applySystemSetting` | `(uint32 id, string key, double value) → bool` | `gainDb` / `reloadOnRomChange` → a `ConfigField` ([`:99`](../packages/native/src/EngineRpcService.cpp#L99)) |
| `applyRoleConfig` | `(uint32 id, string kind, string config) → bool` | `kind=="sameboy"` only; decodes the whole role config, pushes Model/Highpass/LinkGroup/FastBoot ([`:111`](../packages/native/src/EngineRpcService.cpp#L111)) |
| `readState` | `(uint32 id) → optional<Bytestring>` | published savestate from the registry ([`:126`](../packages/native/src/EngineRpcService.cpp#L126)) |
...
| `getFrame` | `(uint32 id) → RpcFrame` | registry frame, raw XRGB8888 ([`:145`](../packages/native/src/EngineRpcService.cpp#L145)) |
| `compileScript` | `(string source) → optional<Bytestring>` | `dsp::compileToBytecode` on a throwaway context ([`:158`](../packages/native/src/EngineRpcService.cpp#L158)) |
...
| `renderAudio` | `(double ms) → Bytestring` | synchronous pull render, interleaved f32 L/R ([`:182`](../packages/native/src/EngineRpcService.cpp#L182)) |
| `setTransport` / `setBpm` | `→ bool` | queued transport ops |
| `setAudioRouting` | `(uint32 mode) → bool` | gated `mode ≤ 2` (Stereo / TwoPerInstance / OnePerInstance) ([`:215`](../packages/native/src/EngineRpcService.cpp#L215)) |
...
([`EngineRpcService.cpp:87`](../packages/native/src/EngineRpcService.cpp#L87)):
...
([`AudioDriverRpcService.hpp:17`](../packages/native/src/AudioDriverRpcService.hpp#L17)).

| Method | Signature | Behaviour |
|---|---|---|
| `startAudio` | `→ bool` | sets `audioThreadOwns(true)` **before** spawning `audioThread_` ([`:55`](../packages/native/src/AudioDriverRpcService.cpp#L55)) |
| `stopAudio` | `→ bool` | clears the bit, joins, then `drainInto` + `reclaimReleased` ([`:67`](../packages/native/src/AudioDriverRpcService.cpp#L67)) |
| `audioCaptured` | `→ AudioCaptured{energy, frames}` | monotonic atomics for a windowed RMS ([`:78`](../packages/native/src/AudioDriverRpcService.cpp#L78)) |
| `sleepMs` | `(double) → bool` | |
| `systemCount` | `→ uint32` | republished atomic while running; live read when quiescent ([`:89`](../packages/native/src/AudioDriverRpcService.cpp#L89)) |
| `drainReleased` | `→ uint32` | control-thread reclaim of released cores ([`:98`](../packages/native/src/AudioDriverRpcService.cpp#L98)) |

The loop ([`audioLoop`](../packages/native/src/AudioDriverRpcService.cpp#L32)) runs
...
[`BackendConstructSpec`](../packages/native/src/BackendTypes.hpp#L29) is the one
...
[`Engine`](../packages/native/src/Engine.hpp#L35) is the single-threaded,
...
Its per-block driver [`Engine::processBlock`](../packages/native/src/Engine.cpp#L74)
...
4. [`registry_.publishAll(...)`](../packages/native/src/Engine.cpp#L97) — copy every
...
[`Engine::applyConfigField`](../packages/native/src/Engine.cpp#L152) — a SameBoy-only
...
[`QueuedInvoker`](../packages/native/src/EngineInvoker.hpp#L28) (in the files
`EngineInvoker.{hpp,cpp}`) is **the one mutation path — there is no Direct/Queued fork**
([`:19`](../packages/native/src/EngineInvoker.hpp#L19)). Every control-plane edit
packs into a POD [`DspCommand`](../packages/native/src/DspCommand.hpp#L19) and pushes
...
([`DspCommand.hpp:19-43`](../packages/native/src/DspCommand.hpp#L19)):
...
[`drainInto`](../packages/native/src/EngineInvoker.cpp#L106) switches over the kind,
...
([`DspEvent.hpp`](../packages/native/src/DspEvent.hpp)); the control thread drains it
in [`reclaimReleased`](../packages/native/src/EngineInvoker.cpp#L162):
...
just-claimed slot ([`:18`](../packages/native/src/EngineInvoker.cpp#L18)); a **full
release ring** logs and **leaks rather than block/free** in the render loop
([`:203`](../packages/native/src/EngineInvoker.cpp#L203)); a full ring on an inline
POD command drops a lost edge, not a leak.
[`freePending`](../packages/native/src/EngineInvoker.cpp#L178) (teardown only, ring
...
[`pluginProcessBlock`](../packages/native/src/BackendFacade.cpp#L35) (`drainInto` →
set bpm/transport direct → `processBlock`) for the plugin, and `audioLoop` for the test host.
`pluginActivate` / `pluginDeactivate`
([`BackendFacade.cpp:19-33`](../packages/native/src/BackendFacade.cpp#L19)) flip the
...
[`SnapshotRegistry`](../packages/native/src/SnapshotRegistry.hpp#L35) is a
fixed-address `std::array<Slot, 64>`
([`:79`](../packages/native/src/SnapshotRegistry.hpp#L79)). Each
[`Slot`](../packages/native/src/SnapshotRegistry.hpp#L66) carries an **atomic `id`**
...
- **`claim(id, sys)`** ([`:22`](../packages/native/src/SnapshotRegistry.cpp#L22)) —
  ...
  `id.store(..., release)` **LAST** ([`:64`](../packages/native/src/SnapshotRegistry.cpp#L64))
...
- **`publishAll(project, frames, sampleRate)`**
  ([`:68`](../packages/native/src/SnapshotRegistry.cpp#L68)) — for each system,
...
- **`release(id)`** ([`:140`](../packages/native/src/SnapshotRegistry.cpp#L140)) —
  clear `id` **FIRST** ([`:147`](../packages/native/src/SnapshotRegistry.cpp#L147)) so
...
([`readState`/`readSram`/`screenshot`/`getFrame`](../packages/native/src/Engine.cpp#L108))
...
([`SnapshotRegistry.hpp:23`](../packages/native/src/SnapshotRegistry.hpp#L23)); that
second copy collapses once a core is legacy-free ([07-migration.md](07-migration.md)).

## The core backends: `SystemFactory` + `SameBoyBackend` / `MesenBackend`

[`SystemFactory`](../packages/native/src/SystemFactory.hpp#L38) is the one build
...
`MesenBackend`](../packages/native/src/BackendFacade.cpp#L8)). `build` dispatches on
the key and returns nullptr on an unknown core.

Every backend consumes a backend-agnostic
[`SystemBuildSpec`](../packages/native/src/SystemFactory.hpp#L17) — `core`,
...
[`SystemBackend::build`](../packages/native/src/SystemFactory.hpp#L29) runs on the
control thread (heavy, non-RT) and returns an **already-`onActivate`'d** system or nullptr. The
wire→build mapping is
[`toBuildSpec`](../packages/native/src/EngineRpcService.cpp#L52): `core` defaults
from `platform` via
[`defaultCoreFor`](../packages/native/src/EngineRpcService.cpp#L43) (nes/gba → mesen,
else sameboy); zip-import seed bytes win over on-disk paths; `settings` passes through unchanged.

### `SameBoyBackend`

[`SameBoyBackend::build`](../packages/native/src/SameBoyBackend.cpp#L42) resolves the
...
[`SameBoyRoleConfig`](../packages/native/src/SameBoyBackend.hpp#L18) and applies
model / highpass / linkGroupId / fastBoot **at construct**
([`:71-74`](../packages/native/src/SameBoyBackend.cpp#L71)) — before `onActivate`, so a
...
[`buildSameBoy`](../packages/native/src/SameBoyBackend.cpp#L31) is the critical seam:

```cpp
auto sys = std::make_unique<SameBoySystem>(id, std::move(cfg), std::move(romBytes));
sys->setSniffDefaultRoles(false);  // cores are bare — feature roles live in the TS kernel
...
`coreRoles.ts`; [`decodeSameBoyRoleConfig`](../packages/native/src/SameBoyBackend.cpp#L26)
...
[`MesenBackend::build`](../packages/native/src/MesenBackend.cpp#L24) is one backend
...
[`DspRuntime`](../packages/native/src/DspRuntime.hpp#L28) owns a **second, bare
...
([`DspRuntime.cpp`](../packages/native/src/DspRuntime.cpp)):

| Thunk | Fills | Fanned to |
|---|---|---|
| [`pushSerialIn(system, frame, byte)`](../packages/native/src/DspRuntime.cpp#L17) | `serialIn_` | the addressed core's serial FIFO |
| [`emitMidiOut(system, frame, [bytes])`](../packages/native/src/DspRuntime.cpp#L32) | `midiOut_` | the host (DAW) after the block |
| [`pressButton(system, frame, button, down)`](../packages/native/src/DspRuntime.cpp#L60) | `buttonOut_` | the addressed core's joypad |
...
with [`JS_UpdateStackTop(rt_)`](../packages/native/src/DspRuntime.cpp#L102) because
...
[`ScriptCompiler`](../packages/native/src/ScriptCompiler.hpp#L17) (`dsp::compileToBytecode`)
...
[`retroplug-cli-core`](../packages/native/CMakeLists.txt#L41) (the static lib that
compiles [`packages/native/src`](../packages/native/src) once) and wraps its **runtime**
classes only. The full shared-vs-legacy split — what survives the legacy deletion, what the host
links-but-doesn't-instantiate, and the naming/rehoming follow-up — is inventoried in
[07-migration.md](07-migration.md). The classes the host actually drives:
...
| [`RomSniffer`](../packages/native/src/system/sameboy/RomSniffer.cpp) | linked, but the host disables its default-role step (`setSniffDefaultRoles(false)`) |
...
One CMake target,
[`retroplug-backend`](../packages/native/CMakeLists.txt#L13), compiles
...
[`dpf_add_plugin(retroplug TARGETS clap vst3 jack)`](../packages/native/CMakeLists.txt#L105).
...
so DPF resolves the host's
[`DistrhoPluginInfo.h`](../packages/native/plugin/DistrhoPluginInfo.h#L8) over the
...
- **Mesen state-snapshot arming.**
  [`SameBoyBackend`](../packages/native/src/SameBoyBackend.cpp#L37) calls
  `enableStateSnapshot()`, but
  [`MesenBackend`](../packages/native/src/MesenBackend.cpp#L24) does **not** — yet
  `constructSystem` calls `registry().claim` for *every* system, and `claim` documents that the
  core must already be armed for `stateRegions()`. So a NES/GBA system gets a registry
  slot with an empty state region: `readSram` and state-based Duplicate are degraded for Mesen
  cores **today** (a present gap — the host builds and arms its own cores, so this is not a
...
- **Live memory-region subscription.** There is no equivalent of the legacy
...
- **Backend role-knobs for Mesen.** `MesenBackend` ignores the `settings` blob; NES/GBA have no
  ...
  [`Engine::applyConfigField`](../packages/native/src/Engine.cpp#L152) is SameBoy-only).
- **Per-block buttons/keys into the kernel.** `Engine::processBlock` passes `kNoButtons` /
  `kNoKeys` today ([`Engine.cpp:18`](../packages/native/src/Engine.cpp#L18)); the
...
- [`BackendFacade.hpp`](../packages/native/src/BackendFacade.hpp) / [`.cpp`](../packages/native/src/BackendFacade.cpp) — the composition root the three hosts share.
- [`BackendRpcRegistration.hpp`](../packages/native/src/BackendRpcRegistration.hpp) — the single source of truth for the wire surface.
- [`Engine.hpp`](../packages/native/src/Engine.hpp) / [`.cpp`](../packages/native/src/Engine.cpp) — the single-threaded owner of the `Project` + DSP kernel.
- [`EngineInvoker.hpp`](../packages/native/src/EngineInvoker.hpp) / [`.cpp`](../packages/native/src/EngineInvoker.cpp) — `QueuedInvoker`, the one mutation path; [`DspCommand.hpp`](../packages/native/src/DspCommand.hpp) / [`DspEvent.hpp`](../packages/native/src/DspEvent.hpp).
- [`SnapshotRegistry.hpp`](../packages/native/src/SnapshotRegistry.hpp) / [`.cpp`](../packages/native/src/SnapshotRegistry.cpp) — the owned read door.
- [`SystemFactory.hpp`](../packages/native/src/SystemFactory.hpp), [`SameBoyBackend.cpp`](../packages/native/src/SameBoyBackend.cpp), [`MesenBackend.cpp`](../packages/native/src/MesenBackend.cpp) — the build path.
- [`DspRuntime.hpp`](../packages/native/src/DspRuntime.hpp) / [`.cpp`](../packages/native/src/DspRuntime.cpp), [`ScriptCompiler.hpp`](../packages/native/src/ScriptCompiler.hpp) — the bare DSP context runner + compiler.
- [`HostRpcService.cpp`](../packages/native/src/HostRpcService.cpp), [`EngineRpcService.cpp`](../packages/native/src/EngineRpcService.cpp), [`AudioDriverRpcService.cpp`](../packages/native/src/AudioDriverRpcService.cpp) — the three concern services.
