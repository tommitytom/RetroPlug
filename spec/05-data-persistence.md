...
| `recent.json` | `RecentEntry[]` ([recentList.ts](../packages/retroplug/src/recentList.ts)) | `RECENT_SCHEMA = 2` | `{ schemaVersion, entries: [{ path, name, song? }] }`, most-recent-first, capped at 10. `name` is the project's name as of the last record: its own name when set, else its primary cart's `"<sav.ext> - <rom>"` identity |
