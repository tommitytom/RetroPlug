...
| `retroplug-cp-bundle` | custom | `build/native-greenfield/cp-bundle_data.c` — control-plane bytecode (`rp_` prefix), embedded in the DSP. | yes |
| `retroplug-ui-bundle` | custom | `build/native-greenfield/ui-bundle_data.c` — React UI bytecode (`rp_` prefix). | no (pulled in by dependents) |
...
`DISTRHO_PLUGIN_NAME "RetroPlug Greenfield"`, URI `urn:distrho:retroplug`, CLAP id
...
5. **React resolution** — the greenfield UI ([ui/main.tsx](../packages/retroplug/ui/main.tsx)) is bundled by the shared [tools/build-ui.js](../tools/build-ui.js), which resolves `react`/`react-reconciler`/`lvgljs-ui` from dpf.js's own pnpm workspace under `deps/dpf.js/deps/lv_binding_js/node_modules`.
...
| **Anything you want to eyeball** in the standalone | `pnpm screenshot` → Read `/tmp/retroplug.png` | Runs the JACK standalone under Xvfb + dummy jackd and dumps the LVGL screen. Drive input mid-run with `tools/standalone-key.sh` using `RETROPLUG_WINDOW_NAME="RetroPlug Greenfield"`. |
...
| **DPF wrapper / plugin-format / state-restore behaviour** | `pnpm validate` | `clap-validator` + `pluginval` against the greenfield `.clap`/`.vst3`. Catches ABI / state-restore / threading regressions in the format adapters. |
| **End-to-end inside a real DAW** | `pnpm reaper:mgb-smoke-greenfield` | Instantiates `retroplug.vst3` in headless Reaper 7.x, plays mGB, renders audio. The first proof the plugin works in a host (not just the harness, which bypasses DPF). |
