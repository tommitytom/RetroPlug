...
| **Bindings / keymap editor UI** | full keyboard+gamepad capture submenu | **Built.** Settings → Keyboard Bindings + Gamepad Bindings ([menuDefs.ts](../packages/retroplug-greenfield/ui/screens/menu/menuDefs.ts) `bindingsChildren` per channel): a profile switcher, one capture row per GB button (press a key / a controller button / a stick flick to rebind, Backspace clears), a channel reset, and New / Rename / Delete, write-through via `bindingsStore`. |
...
| [menuDefs.ts](../packages/retroplug-greenfield/ui/screens/menu/menuDefs.ts) (About panel marker) | Deferred About panel only — LSDj mode, live gamepad input, and the keyboard + gamepad rebinding editors (with analog-stick-as-dpad) are all now built |
