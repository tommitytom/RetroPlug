...
| **Built** | **Breakpoints + `runUntilBreak`** — exec/read/write watchpoints with expression conditions (`[$40F0]!=0`, `Y==0`, scanline/cycle gates) | `Breakpoint::Create` → `Debugger::SetBreakpoints`; `ExpressionEvaluator` | `setBreakpoints(id, Breakpoint[]): bool` + `runUntilBreak(id, maxCycles): BreakInfo`. The `$40F1` read-watch fires. Lazily inits the heavy debugger. The NES expression tokens are `value` / `address` / `iswrite` / `oppc` plus `frame` / `cycle` / `scanline` / `sprite0hit` / `spriteoverflow` (`ExpressionEvaluator.Nes.cpp`): the last group is what makes a PPU-timing condition expressible. Prefer `drainBreakHits` (below) unless you actually want to stop. |
...
| **Built** | **Core→host raw bytes** — the direction `stageMidiIn` doesn't cover. On the NES the ROM frames a `CMD_USB_WR` to `$40F0` and the cartridge hands the payload to the host (on hardware the N8's MCU forwards it out of the USB port; `retroplug-n8-hwtest fiford` reads the same stream). Lets a ROM being streamed to acknowledge what it consumed, so the host paces to it instead of guessing. | Nothing - RetroPlug's own [NesEverdriveFifo](../packages/native/src/system/mesen/NesEverdriveFifo.hpp), which parses `$40F0` writes as Edio commands exactly as the device's MCU does | `drainCoreBytes(id): Uint8Array` — a `SystemBase` virtual (`drainCoreBytes`, the inverse of the DSP kernel's `pushCoreBytes`), so no debug target is created. Take-not-peek; empty for a core with no such transport. The same FIFO serves the emulated **SD card**, whose root is the `mesen` role's `sdRoot` (spec/04), and the `CMD_F_FRD_MEM` **DMA** that reads a file off it straight into CHR-RAM (observable with `readMemory(id, MemoryRegion.Vram)`; the `$40FF` wait it makes the ROM do is modelled from the hardware transfer rate). |
| **Built** | **Transport depth + loss**: what that FIFO is holding and what it has DROPPED. The emulated queue is unbounded and instant by default, so a ROM can never be starved and never loses a byte; `setRoleConfig(id, "mesen", { fifo: "hardware" })` + `reset(id)` gives it the cartridge's 2048-byte depth and ~150 KB/s wire (spec/04). Both absences hide a bug class: a ROM that cannot lose a byte never has its recovery path exercised, and one handed a chunk atomically is never starved mid-structure. | `NesEverdriveFifo`'s two stages (the wire, then fifo_a) | `getCoreTransportStats(id): { rxDepth, rxCapacity, droppedBytes, wirePending, txDepth }`. `droppedBytes` is cumulative (a read does not clear it), so sample and subtract; plotting `rxDepth` over a run is how a starvation bug shows itself. |
| **Built** | **Passive breakpoint capture**: the firings recorded during an ORDINARY render, not under `runUntilBreak`. Mesen's headless `SleepUntilResume` records a break and returns rather than blocking, and `MesenNesSystem::runUnit` already steps one instruction at a time, so the render loop notes the hit and resumes. That is what makes a Mesen condition a **continuous invariant** instead of something only a single-stepping run can see. | `Debugger::IsExecutionStopped` + `GetLastBreakEvent` polled in the render loop (armed only while a breakpoint set is installed) | `drainBreakHits(id): { hits, overflow }`, each hit carries `pc`, `address`, `value`, `scanline`, `cycle`, `cpuCycle`. Wrapped by `renderTimeline(s, tl, { invariants: [{ system, symbol, max }] })`, which resolves the symbol, installs the watchpoint, and throws with the frame position of the first violation. |
