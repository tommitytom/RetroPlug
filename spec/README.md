...
This directory is the single source of truth for the RetroPlug2 architecture — the
active implementation in [`packages/native/`](../packages/native) (C++ host)
...
RetroPlug2 is mid-port. Two plugin builds coexist in the repo today: the current build (documented
...
[`packages/cli/`](../packages/cli)). Legacy still ships only because the current build has not yet reached
full parity. **Legacy is being removed** — the current build will become the sole build. These docs
describe the current build; legacy appears only where a doc must note it is going
...
| Doc | What it covers |
|---|---|
| [00-overview.md](00-overview.md) | The front door: what RetroPlug2 is, the thesis, the artifact set (clap/vst3/jack + standalone + test hosts), the package layout, a one-screen component map. |
...
| [02-native-host.md](02-native-host.md) | The C++ host (`packages/native`): the full RPC surface, `Engine` + `QueuedInvoker`, `SnapshotRegistry` internals, `SystemFactory` + the core backends, the bare DSP context runner, and the shared-core classes it wraps. |
...
| [07-migration.md](07-migration.md) | The switchover to a single sole build: the feature gap, the shared-vs-legacy C++ map, the rename/delete checklist, the gaps the build must own, and the code-comment cleanup follow-up. |
