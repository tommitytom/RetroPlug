...
| [12-dynamic-parameters.md](12-dynamic-parameters.md) | **Planned, not built.** Per-ROM named DAW automation lanes (mGB / BlipToaster MIDI CCs) for CLAP + VST3: why DPF cannot do it today, the fixed-pool / mutable-descriptor contract the two formats actually permit, the DPF fork changes behind a new opt-in macro, the RetroPlug seam, and the real-DAW verification. |
