...
namespace rp {
	...
	orb::ViewPtr RetroPlugApplication::onCreateUi() {
		return std::make_shared<RetroPlugView>(getProject());
	}

	orb::ViewPtr RetroPlugApplication::onCreateNamedView(const std::string& name) {
		if (name == "HexEditor") {
	...
	orb::AudioProcessorPtr RetroPlugApplication::onCreateAudio() {
