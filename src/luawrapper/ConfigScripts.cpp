// GENERATED! CHANGES WILL BE OVERWRITTEN!

#include "ConfigScripts.h"

std::array<RawScript, 2> _rawScripts = {
	RawScript { "config.lua", R"(KeyMap({
	[Key.D] = Button.A,
	[Key.W] = Button.B,
...
PadMap({ romName = "LSDj*" }, {
	...
	RawScript { "input/default.lua", R"(--[[InputConfig({
	name = "Default",
	author = "tommitytom"
})]]
...
const std::array<RawScript, 2>& getRawScripts() { return _rawScripts; }
