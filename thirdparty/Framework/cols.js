...
let out = 'local colors = {\n';

for (const item of items) {
	const idx1 = item.rgb.indexOf(',', 4);
	const idx2 = item.rgb.indexOf(',', idx1);
	const idx3 = item.rgb.indexOf(')', idx2);

	const r = parseInt(item.rgb.substring(4, idx1)) / 255;
	const g = parseInt(item.rgb.substring(idx1 + 1, idx2)) / 255;
	const b = parseInt(item.rgb.substring(idx2 + 1, idx3)) / 255;

	//out += `\t${item.name.toLowerCase()} = fw.Color4F(${r}, ${g}, ${b}),\n`;
	out += `\t${item.name.toLowerCase()} = 0x${item.hex.substring(1)}FF,\n`;
}
