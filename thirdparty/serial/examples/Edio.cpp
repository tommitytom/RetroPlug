...
void Edio::seek() {
	...
	for (auto& p : ports) {
		try {
			openConnection(p.port);
			return;
		} catch (const std::exception&) {}
	}
	throw std::runtime_error("EverDrive not found");
}

void Edio::openConnection(const std::string& portName) {
	try {
...
void Edio::setTimeout(uint32_t ms) {
	port->setTimeout(serial::Timeout::simpleTimeout(ms));
}
