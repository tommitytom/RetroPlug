-- reaper-risa-author.lua
--
-- Author the risa host-sync DAW-timing project and save it. The NES counterpart of
-- reaper-lsdj-author.lua, and deliberately simpler: risa host sync is driven by the TRANSPORT alone,
-- so the plugin track carries NO MIDI item. RetroPlug's rom provider attaches `risa-sync` off the
-- ROM's RISAxyz marker when the autoloaded .rplg constructs, and the role turns the transport into
-- risa's arm / start / 24-PPQN clock / stop byte stream.
--
-- Same two-track shape as the LSDj renders: track 1 = the plugin ("RetroPlug", pan hard-L), track 2 =
-- ReaSynth click (pan hard-R, one short C-5 per quarter beat), so the stereo analyzer separates risa (L)
-- from the DAW's beat grid (R). Letting Reaper add the FX by name captures the correct scanned VST3
-- GUID. The risa project itself loads from the autoloaded .rplg (RETROPLUG_AUTOLOAD_PROJECT), whose
-- song hits the noise channel once per beat, so every beat is a transient the drift analyzer can pair.

local dest       = os.getenv("REAPER_AUTHOR_DEST") or "/tmp/risa.rpp"
...
local RENDER_STEM = "reaper-risa-sync"
