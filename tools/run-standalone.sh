#!/usr/bin/env bash
#
# Run the retroplug standalone (JACK target) headlessly:
#   * Xvfb provides a virtual display
#   * jackd dummy backend provides an audio server with no real I/O
#   * the screenshot env var dumps PNGs of the LVGL screen periodically
#
# Designed for agent workflows on hosts with no display or audio hardware.
# Cleans up Xvfb/jackd on exit.
#
# Usage:
#   tools/run-standalone.sh                      # screenshot to /tmp/retroplug.png after 3s
#   tools/run-standalone.sh /tmp/x.png 5         # custom path + 5s run
#   tools/run-standalone.sh /tmp/x.png 5 250     # 5s with 250ms screenshot interval
#
# Optional: send keystrokes during the run via tools/standalone-key.sh
# (xdotool against the same DISPLAY).
#
# Dependencies (one-time):
#   sudo apt-get install xvfb jackd2 xdotool

set -euo pipefail

OUT="${1:-/tmp/retroplug.png}"
DURATION="${2:-3}"
INTERVAL="${3:-1000}"

# Locate the repo root regardless of where the script is invoked from.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$REPO_DIR"

# Tool checks
for cmd in Xvfb jackd; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        echo "error: '$cmd' not installed. Run:" >&2
        echo "  sudo apt-get install xvfb jackd2 xdotool" >&2
        exit 1
    fi
done

# Build the standalone if missing.
if [ ! -x "$REPO_DIR/build/bin/retroplug" ]; then
    echo "building standalone..."
    make -C "$REPO_DIR/build" -j"$(nproc)" retroplug >/dev/null
fi

# Find a free X display.
DISP=99
while [ -e "/tmp/.X${DISP}-lock" ]; do DISP=$((DISP + 1)); done
export DISPLAY=":${DISP}"

# Start Xvfb.
Xvfb "$DISPLAY" -screen 0 1024x768x24 -nolisten tcp >/dev/null 2>&1 &
XVFB_PID=$!

# Start jackd with dummy backend (no real audio I/O).
jackd -d dummy -r 44100 -p 1024 >/tmp/retroplug-jackd.log 2>&1 &
JACK_PID=$!

# Brief wait for both servers to be ready.
sleep 0.4

cleanup() {
    [ -n "${RETRO_PID:-}" ] && kill "$RETRO_PID" 2>/dev/null || true
    kill "$JACK_PID" 2>/dev/null || true
    kill "$XVFB_PID" 2>/dev/null || true
    wait 2>/dev/null || true
}
trap cleanup EXIT INT TERM

# Run the standalone with the screenshot hook armed.
RETROPLUG_SCREENSHOT_PATH="$OUT" \
RETROPLUG_SCREENSHOT_INTERVAL_MS="$INTERVAL" \
    "$REPO_DIR/build/bin/retroplug" >/tmp/retroplug-stdout.log 2>&1 &
RETRO_PID=$!

# Let it run.
sleep "$DURATION"

# trap cleanup runs

if [ -f "$OUT" ]; then
    echo "screenshot: $OUT ($(stat -c '%s bytes' "$OUT"))"
else
    echo "warning: no screenshot produced at $OUT" >&2
    echo "check stdout: /tmp/retroplug-stdout.log" >&2
    echo "check jackd:  /tmp/retroplug-jackd.log" >&2
    exit 1
fi
